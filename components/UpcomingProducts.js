...
const ProductComponent = ({ product }) => {
  ...
  const handlePress = () => {
    console.log(`Navigating to ProductDetailPage with productId: ${product.productId}`);
...
const UpcomingProducts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('https://crossbee-server.vercel.app/upcomingProducts');
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>UpcomingProducts</Text>
      <ScrollView
        horizontal
