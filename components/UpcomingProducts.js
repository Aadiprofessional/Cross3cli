...
const ProductComponent = ({product}) => {
  ...
  const { wishlist, addToWishlist, removeFromWishlist } = useWishlist(); // Using wishlist context
...
const UpcomingProducts = () => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        const response = await fetch(
          'https://crossbee-server-1036279390366.asia-south1.run.app/upcomingProducts?uid='+ userId,
        );
  ...
  return (
    <View style={styles2.container}>
      <Text style={styles2.title}>Upcoming Products</Text>
      <ScrollView
        horizontal
